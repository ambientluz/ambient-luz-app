...
function gerarHTMLCliente(proposta) {
  ...
  const extrasHTML = extras.length > 0 ? `
    <div style="margin-top:20px;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
      ...
      ${extras.map((e, ii) => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:${ii%2===0?'#fff':'#f9fafb'};border-bottom:1px solid #f3f4f6;">
...
function AbaGestor({ historico, setHistorico }) {
  ...
  const custo = parseFloat(String(custoInput).replace(",",".")) || 0;
  const desconto = parseFloat(String(descontoInput).replace(",",".")) || 0;
  const totalBruto = proposta?.total || 0;
  const totalDesconto = totalBruto*(1-desconto/100);
  const lucroGross = totalDesconto-custo;
  const margemGross = totalDesconto>0&&custo>0 ? (lucroGross/totalDesconto)*100 : null;

  useEffect(() => {
    if (proposta?.custo) { setCustoInput(String(proposta.custo)); setDescontoInput(String(proposta.desconto||0)); }
    else { setCustoInput(""); setDescontoInput(""); }
  }, [selecionada]);
    ...
    <div style={{ padding:"24px 32px" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:24 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:13, color:"#6b7280" }}>{proposta.data} · {proposta.vendedor}</div>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20 }}>
        <div>
          ...
          <input type="text" value={custoInput} onChange={e => setCustoInput(e.target.value)} placeholder="Ex: 13000"
            style={{ width:"100%", padding:"10px 14px", border:"1px solid #d1d5db", borderRadius:8, fontSize:15, outline:"none", boxSizing:"border-box" }} />
        </div>
        <div>
          <label style={{ fontSize:12, fontWeight:600, color:"#6b7280", display:"block", marginBottom:6, textTransform:"uppercase", letterSpacing:"0.05em" }}>Desconto aplicado (%)</label>
          <input type="text" value={descontoInput} onChange={e => setDescontoInput(e.target.value)} placeholder="0"
            style={{ width:"100%", padding:"10px 14px", border:"1px solid #d1d5db", borderRadius:8, fontSize:15, outline:"none", boxSizing:"border-box" }} />
        </div>
      </div>
      ...
      {custo>0 && (
        <>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:28 }}>
            {[
              {label:"Total bruto", val:fmtBRL(totalBruto), cor:"#1a1a1a"},
              {label:desconto>0?`Após ${desconto}% desc.`:"Sem desconto", val:fmtBRL(totalDesconto), cor:"#1a1a1a"},
              {label:"Margem bruta", val:margemGross!=null?`${margemGross.toFixed(1)}%`:"—", cor:margemGross!=null?margemCor(margemGross):"#1a1a1a"},
              {label:"Lucro bruto", val:fmtBRL(lucroGross), cor:lucroGross>=0?"#16a34a":"#dc2626"},
            ].map((c,i) => (
            ...
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              ...
              <tbody>
                {TAXAS.map((t,i) => {
                  ...
                  const liquido = base*(1-t.taxa);
                  const lucroLiq = liquido-custo;
                  const margemLiq = (lucroLiq/liquido)*100;
                  const cor = margemCor(margemLiq);
                  return (
                    <tr key={i} style={{ borderBottom:i<TAXAS.length-1?"1px solid #f3f4f6":"none", background:i%2===0?"#fff":"#fafafa" }}>
                      <td style={{ padding:"9px 14px", fontSize:13, fontWeight:500 }}>{t.label}</td>
                      <td style={{ padding:"9px 14px", fontSize:13, color:"#6b7280", textAlign:"right" }}>{t.taxa>0?`${(t.taxa*100).toFixed(2)}%`:"—"}</td>
                      <td style={{ padding:"9px 14px", fontSize:13, textAlign:"right" }}>{fmtBRL(liquido)}</td>
                      <td style={{ padding:"9px 14px", fontSize:13, textAlign:"right", color:lucroLiq>=0?"#16a34a":"#dc2626", fontWeight:500 }}>{fmtBRL(lucroLiq)}</td>
                      <td style={{ padding:"9px 14px", textAlign:"right" }}>
                        <span style={{ background:cor+"18", color:cor, fontWeight:700, fontSize:13, padding:"3px 10px", borderRadius:6 }}>{margemLiq.toFixed(1)}%</span>
                      </td>
                    </tr>
                  );
                })}
          ...
          <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:8, padding:"12px 16px", fontSize:13, color:"#166534" }}>
            <strong>Meta 70% de margem:</strong> custo máximo {fmtBRL(totalDesconto*0.30)} · markup mínimo 3,33x
          </div>
        </>
      )}
    </div>
  ...
  return (
    <div style={{ padding:"24px 32px" }}>
      ...
      <p style={{ fontSize:13, color:"#6b7280", marginBottom:20 }}>Clique para ver a análise de margem.</p>
      {historico.slice().reverse().map((p,i) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              {temCusto?(
                <span style={{ background:margemCor(margem)+"18", color:margemCor(margem), fontWeight:700, fontSize:13, padding:"4px 12px", borderRadius:6 }}>{margem.toFixed(1)}% margem</span>
              ):(
                <span style={{ background:"#fef3c7", color:"#92400e", fontSize:12, fontWeight:600, padding:"4px 10px", borderRadius:6 }}>Sem custo</span>
              )}
